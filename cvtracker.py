...
detector_face_mesh = FaceMeshDetector(staticMode=False, maxFaces=2, minDetectionCon=0.5, minTrackCon=0.5)

# Initialize the FaceDetector class with the given parameters
detector_face_box = FaceDetector(minDetectionCon=0.5, modelSelection=0)
...
while True:
    ...
    img, face_boxes = detector_face_box.findFaces(img, draw=True)
    img, faces = detector_face_mesh.findFaceMesh(img, draw=True)
    ...
    if hands:
        ...
        length, info, img = detector_hand.findDistance(lmList1[8][0:2], lmList1[12][0:2], img, color=(255, 0, 255),
                                                  scale=10)
        ...
        if len(hands) == 2:
            ...
            length, info, img = detector_hand.findDistance(lmList1[8][0:2], lmList2[8][0:2], img, color=(255, 0, 0),
                                                      scale=10)
    ...
    if faces:
        ...
        for face in faces:
            ...
            leftEyeVerticalDistance, info = detector_face_mesh.findDistance(leftEyeUpPoint, leftEyeDownPoint)
