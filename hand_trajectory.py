        ...
        for hand in frame["hands"]:
            ...
            for i, landmark in enumerate(hand_marks):
                if i not in landmark_trajectories[hand_key]:
                    landmark_trajectories[hand_key][i] = {"x": [], "y": [], "z": [], "frame": []}
                landmark_trajectories[hand_key][i]["x"].append(landmark["x"])
                landmark_trajectories[hand_key][i]["y"].append(landmark["y"])
                landmark_trajectories[hand_key][i]["z"].append(landmark["z"])
                landmark_trajectories[hand_key][i]["frame"].append(frame["frame"])
...
def display_dynamic(landmark_trajectories, interval=50):
    ...
    if landmark_trajectories["Left"].values():
        x_min = min(min(traj["x"]) for traj in landmark_trajectories["Left"].values())
        x_max = max(max(traj["x"]) for traj in landmark_trajectories["Left"].values())
        y_min = min(min(traj["y"]) for traj in landmark_trajectories["Left"].values())
        y_max = max(max(traj["y"]) for traj in landmark_trajectories["Left"].values())
        z_min = min(min(traj["z"]) for traj in landmark_trajectories["Left"].values())
        z_max = max(max(traj["z"]) for traj in landmark_trajectories["Left"].values())

    # Consider the right hand as well
    if landmark_trajectories["Right"].values():
        x_min = min(x_min, min(min(traj["x"]) for traj in landmark_trajectories["Right"].values()))
        x_max = max(x_max, max(max(traj["x"]) for traj in landmark_trajectories["Right"].values()))
        y_min = min(y_min, min(min(traj["y"]) for traj in landmark_trajectories["Right"].values()))
        y_max = max(y_max, max(max(traj["y"]) for traj in landmark_trajectories["Right"].values()))
        z_min = min(z_min, min(min(traj["z"]) for traj in landmark_trajectories["Right"].values()))
        z_max = max(z_max, max(max(traj["z"]) for traj in landmark_trajectories["Right"].values()))
    ...
    max_frames = 0
    if landmark_trajectories["Left"].values():
        max_frames = max(max_frames, max(len(landmark["frame"]) for landmark in landmark_trajectories["Left"].values()))
    if landmark_trajectories["Right"].values():
        max_frames = max(max_frames, max(len(landmark["frame"]) for landmark in landmark_trajectories["Right"].values()))
    ...
    def update(frame):
        ...
        for chain in finger_chains:
            if landmark_trajectories["Left"].values():
                if frame < len(landmark_trajectories["Left"][0]["x"]):
            ...
            if landmark_trajectories["Right"].values():
                if frame < len(landmark_trajectories["Right"][0]["x"]):
